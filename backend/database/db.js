...
// Додавання тестових інвестиційних продуктів
const seedProducts = () => {
...
// Функція для встановлення ролі адміністратора
const setAdminRole = (email) => {
  try {
    // Спочатку перевіряємо чи існує колонка role
    ...
    if (hasRoleColumn) {
      ...
      if (result.changes > 0) {
        console.log(`✅ Користувач ${email} тепер адміністратор`);
      }
    }
  } catch (error) {
    // Ігноруємо помилку, якщо колонка ще не існує
  }
};

// Встановлення адміністратора для вашого акаунту
setTimeout(() => {
  setAdminRole('pilpandr79@icloud.com');
}, 1000);
